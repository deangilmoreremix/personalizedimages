...
const TVShowActionFigureGenerator: React.FC<TVShowActionFigureGeneratorProps> = ({ tokens, onImageGenerated }) => {
  ...
  const handleRandomize = () => {
    ...
    // Select random additions
    const currentPrompt = tvPrompts[randomPromptIndex];
    const randomAdditions = currentPrompt.additions
      .sort(() => Math.random() - 0.5)
      .slice(0, Math.floor(Math.random() * 3));
    setSelectedAdditions(randomAdditions);
    
    // Select random pose
    const randomPose = currentPrompt.poses[Math.floor(Math.random() * currentPrompt.poses.length)];
